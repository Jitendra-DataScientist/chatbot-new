        ...
        try:
            ...
            if selected_chart is None:
                ...
                try:
                    ...
                    # 🔍 DEBUG: Log context before passing to data_exploration
                    master_logger.info(f"🔍 DEBUG: Context being passed to data_exploration:")
                    master_logger.info(f"  - context type: {type(context)}")
                    master_logger.info(f"  - context value: {context}")
                    if context and isinstance(context, dict):
                        master_logger.info(f"  - workbook_name in context: {context.get('workbook_name')}")
                    elif context and hasattr(context, 'workbook_name'):
                        master_logger.info(f"  - workbook_name in context: {context.workbook_name}")
...
"""
    ...
    async def _no_chart_exploration_node(self, state: QueryProcessingState) -> QueryProcessingState:
        ...
        try:
            ...
            # 🔍 DEBUG: Log context before passing to data_exploration (LangGraph node)
            context_value = state.get("context")
            master_logger.info(f"🔍 DEBUG: Context being passed to data_exploration (LangGraph):")
            master_logger.info(f"  - context type: {type(context_value)}")
            master_logger.info(f"  - context value: {context_value}")
            if context_value and isinstance(context_value, dict):
                master_logger.info(f"  - workbook_name in context: {context_value.get('workbook_name')}")
            elif context_value and hasattr(context_value, 'workbook_name'):
                master_logger.info(f"  - workbook_name in context: {context_value.workbook_name}")
            ...
            result = await exploration_service.process(
                ...
                intent_result=minimal_intent,
                chart_context=None,
                context=state.get("context")  # 🆕 Pass context from state
            )
